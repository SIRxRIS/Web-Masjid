...
export async function getDashboardData(tahun: number, bulan: number) {
  try {
    ...
    const pertumbuhanDanaBulanan = await getPertumbuhanDanaBulanan(tahun, bulan);
...
async function getPertumbuhanDonatur(tahun: number, bulanIni: number): Promise<number> {
  try {
    ...
    const donaturBulanSebelumnya = dataBulanSebelumnya ? dataBulanSebelumnya.length : 0;
    
    // Menghitung persentase pertumbuhan
    if (donaturBulanSebelumnya === 0) return 100; // Jika sebelumnya 0, pertumbuhan 100%
    
    return parseFloat((((donaturBulanIni - donaturBulanSebelumnya) / donaturBulanSebelumnya) * 100).toFixed(1));
  } catch (error) {
...
async function getPertumbuhanDanaTahunan(tahun: number): Promise<number> {
  try {
    ...
    return parseFloat(
      (((pemasukanTahunIni - pemasukanTahunLalu) / pemasukanTahunLalu) * 100).toFixed(1)
    );
  } catch (error) {
...
async function getPertumbuhanDanaBulanan(tahun: number, bulanIni: number): Promise<number> {
  try {
